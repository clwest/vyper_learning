...
# This may only be called bu the chairperson
@external
def giveRightToVote(voter: address):
    ...
    # Throws if the voters voting weight isn't 0
    ...
    for i in range(4):
        if self._delegated(target):
            target = self.voters[target].delegate
             ...
             # or even infinitely with a while loops
             ...
             # the responsibility of the contract client and this
...
# Delegate your vote to the voters 'to'
@external
def delegate(to: address):
    #Throws if the sender has already voted
    assert not self.voters[msg.sender].voted
    #Throws if the sender tried to delegate their vote to themselves or
...
def vote(proposal: int128):
    # can't vote twice
    assert not self.voters[msg.sender].voted
    # can only vote on legitimate proposal
    assert proposal < self.int128Proposals

    self.voters[msg.sender].vote = proposal
    self.voters[msg.sender].voted = True

    # Transfer msg.senders weight to proposal
    self.proposals[proposal].voteCount += self.voters[msg.sender].weight
    self.voters[msg.sender].weight = 0

# Computes the winning prooposal taking all
# pervios votes into account
@view
@internal
